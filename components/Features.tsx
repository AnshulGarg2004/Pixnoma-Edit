    ...
    <div className='max-w-7xl mx-auto px-6'>
        ...
        <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-8'>
            {features.map((feat, index) => (
                <FeatureCards key={index} {...feat} delay={index*100} />
            ))}
        </div>
    </div>
</section>
...
const FeatureCards = ({icon, title, description, delay = 0} : FeatureCardProps) => {
    const [ref, isVisible] = useIntersectionObserver<HTMLDivElement>();
  const [isHovered, setIsHovered] = useState(false);
    return (
    <div ref={ref} className={` relative backdrop-blur-lg  border  rounded-3xl p-8 transition-all duration-700 cursor-pointer ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} ${isHovered ? 'transform scale-115 rotate-1 z-10' : ''} `}
